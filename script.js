...
// ВИТЯГУЄ ТІЛЬКИ ЕМОДЗІ ДЛЯ ШАПКИ
...
function renderTerminal() {
    ...
    if (cyberPages.pages && cyberPages.pages[currentPage]) {
        cyberPages.pages[currentPage].buttons.forEach(btn => {
            ...
            wrapper.dataset.loc = currentPage; // Локація для drag & drop
...
// ОКО ЮЗЕРА (ПОВНЕ РЕДАГУВАННЯ ФОНІВ І КНОПОК ДЛЯ АДМІНА)
function openUserEyeStudio() {
    ...
    // В Оці Юзера фон завжди глобальний (щоб бачити як на головній)
    const globalBg = cyberPages.pages_bg && cyberPages.pages_bg['global'];
...
function changeBackground(event) {
    ...
    if (access === 'admin_king') {
        // Визначаємо на якій сторінці ми зараз
...
function updateMsg(id, newHtml) { const msgDiv = document.querySelector(`.msg[data-id="${id}"]`); if (msgDiv) { msgDiv.innerHTML = newHtml; let history = JSON.parse(localStorage.getItem(CHAT_KEY)) || []; leBtn.className = 'menu-item secret-btn';
