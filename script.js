...
// База даних з підтримкою вкладених сторінок та індивідуальних фонів
let cyberPages = { main: { buttons: [] }, burger: { buttons: [] }, pages: {}, pages_bg: {} };
let isEditMode = false;
let navStack = []; // Пам'ять кроків (Стек навігації)

// --- РОЗШИФРОВКА БАЗИ КНОПОК (БЕЗ ІЄРОГЛІФІВ) ---
if (encodedData) {
    try {
        let decodedString;
...
function openUserEyeStudio() {
    ...
    modal.style.backgroundImage = globalBgValue ? `url('${globalBgValue}')` : 'none';
...
// --- РЕЖИМ КОНСТРУКТОРА ТА СЕНСОРНЕ ПЕРЕТЯГУВАННЯ ---
